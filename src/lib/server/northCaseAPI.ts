import { SavingsDashboardData } from "./types" 

const ENDPOINT_URL = "https://north-case-api.vercel.app/api"

export async function fetchSavingsDashboardData(apiKey: string): Promise<
  SavingsDashboardData
> {
  let response = await fetch(ENDPOINT_URL, {headers: {"Authorization": apiKey}})
  if (!response.ok) {
